...
public class MortarTower : Tower
{
  ...
  float launchingProgress;

  [SerializeField, Range(0.5f, 3f)]
  float shellBlastRadius = 1f;

  [SerializeField, Range(1f, 100f)]
  float shellDemage = 10f;
  ...
  {
    float x = targatingRange + 0.25001f;
  ...
  {
    launchingProgress += shotsPerSecond * Time.deltaTime;
    while (launchingProgress >= 1f)
    {
      if (AcquireTarget(out TargetPoint target))
      {
        Launch(target);
        launchingProgress -= 1f;
      }
      else
      {
        launchingProgress = 0.999f;
      }
    }
  }
